...
interface UploadScreenProps {
  onComplete: (images: ImageUpload[]) => void;
  onBack: () => void;
}
...
export default function UploadScreen({ onComplete, onBack }: UploadScreenProps) {
  ...
  const processFiles = async (files: FileList | null) => {
    ...
    const newImages: ImageUpload[] = await Promise.all(
      validFiles.map(async (file, idx) => {
        const preview = await fileToBase64(file);
        return {
          id: `img_${Date.now()}_${idx}`,
          file,
          preview, // Now actual base64 data URL
          name: file.name,
          size: file.size,
        };
      })
  ...
  const handleAnalyze = () => {
    if (images.length >= 2 && images.length <= 30) {
      onComplete(images);
    }
  };

  const isValidCount = images.length >= 2 && images.length <= 30;
    ...
    <div className="min-h-screen flex flex-col px-6 py-12">
      ...
      <div className="max-w-5xl w-full mx-auto mb-8">
        ...
        <p className="text-slate-400 text-lg">
          Upload 2–30 images to get started. We support JPEG and PNG formats.
        </p>
      </div>
                  ...
                  <div className="text-slate-300">
                    <span
                      className={`text-3xl font-bold ${
                        isValidCount ? 'text-emerald-400' : 'text-amber-400'
                      }`}
                    >
                      {images.length}
                    </span>
