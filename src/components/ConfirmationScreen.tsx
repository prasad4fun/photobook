...
import { Theme, OrderDetails } from '../types';
...
export default function ConfirmationScreen({ selectedTheme, imageCount, onConfirm, onBack }: ConfirmationScreenProps) {
  const [selectedPackage, setSelectedPackage] = useState<'digital' | 'photobook'>('digital');
  const [isProcessing, setIsProcessing] = useState(false);

  const packages: Record<'digital' | 'photobook', {
    id: string;
    name: string;
    price: number;
    features: string[];
    icon: typeof Download | typeof Package;
    badge?: string;
  }> = {
    digital: {
      id: 'digital',
            ...
            <div className="space-y-4 mb-8">
              {Object.values(packages).map((pkg) => {
                ...
                return (
                  <div
                    key={pkg.id}
                    onClick={() => setSelectedPackage(pkg.id as 'digital' | 'photobook')}
